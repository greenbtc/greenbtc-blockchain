...
git submodule update --init greenbtc-blockchain-gui

Set-Location -Path ".\greenbtc-blockchain-gui" -PassThru
...
# Other than `greenbtc-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @greenbtc-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@greenbtc-network` because it causes an error on later `electron-packager` command
#Remove-Item "@greenbtc-network" -Recurse -Force
