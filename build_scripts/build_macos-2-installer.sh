...
LAST_EXIT_CODE=$?
ls -l dist/mac*/greenbtc.app/Contents/Resources/app.asar
...
DMG_NAME="greenbtc-${GREENBTC_INSTALLER_VERSION}.dmg"
if [ "$(arch)" = "arm64" ]; then
  mv dist/${DMG_NAME} dist/greenbtc-${GREENBTC_INSTALLER_VERSION}-arm64.dmg
  DMG_NAME=greenbtc-${GREENBTC_INSTALLER_VERSION}-arm64.dmg
fi
...
if [ "$NOTARIZE" == true ]; then
  ...
  notarize-cli --file="$DMG_NAME" --bundle-id top.greenbtc.blockchain \
...
# xcrun altool --notarize-app -f GreenBTC-0.1.X.dmg --primary-bundle-id top.greenbtc.blockchain -u username -p password
...
# xcrun stapler staple GreenBTC-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate GreenBTC-0.1.X.dmg
