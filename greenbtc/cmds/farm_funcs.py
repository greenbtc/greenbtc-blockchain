...
from greenbtc.util.misc import format_bytes
from greenbtc.util.misc import format_minutes
from greenbtc.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total greenbtc farmed: {amounts['farmed_amount'] / units['greenbtc']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['greenbtc']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['greenbtc']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print(
                "For details on farmed rewards and fees you should run 'greenbtc start wallet' and 'greenbtc wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'greenbtc wallet show'")
    else:
        print("Note: log into your key using 'greenbtc wallet show' to see rewards for each key")
