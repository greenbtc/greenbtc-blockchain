...
from greenbtc.cmds.passphrase_funcs import obtain_current_passphrase
...
from greenbtc.util.errors import KeychainException
from greenbtc.util.file_keyring import MAX_LABEL_LENGTH
from greenbtc.util.ints import uint32
from greenbtc.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
from greenbtc.util.keyring_wrapper import KeyringWrapper
...
def generate_and_print():
    ...
    print(mnemonic)
    print("Note that this key has not been added to the keychain. Run greenbtc keys add")
    return mnemonic
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'greenbtc keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from greenbtc.wallet.derive_keys import _derive_path, _derive_path_unhardened
...
async def migrate_keys(root_path: Path, forced: bool = False) -> bool:
    from greenbtc.util.keyring_wrapper import KeyringWrapper
    from greenbtc.util.misc import prompt_yes_no

    deprecation_message = (
        "\nLegacy keyring support is deprecated and will be removed in an upcoming version. "
        "You need to migrate your keyring to continue using GreenBTC.\n"
    )
