...
from greenbtc.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate
from greenbtc.rpc.data_layer_rpc_client import DataLayerRpcClient
...
from greenbtc.rpc.harvester_rpc_client import HarvesterRpcClient
...
from greenbtc.types.mempool_submission_status import MempoolSubmissionStatus
...
from greenbtc.util.keychain import KeyData
...
from greenbtc.wallet.util.tx_config import CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig, TXConfigLoader
...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'greenbtc wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status"
    ...
    try:
        if fingerprint is not None:
            ...
            if len(fingerprints) == 0:
                raise CliRpcConnectionError("No keys loaded. Run 'greenbtc keys generate' or import a key")
        ...
        click.option(
            ...
            click.option(
                ...
                click.option(
                    ...
                    "amounts_to_exclude",
                    multiple=True,
                    help="Exclude any coins with this GBTC or CAT amount from being included.",
                )(func)
            )
        )
    )
