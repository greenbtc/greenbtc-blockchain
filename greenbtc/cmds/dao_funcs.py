...
from greenbtc.cmds.cmds_util import CMDTXConfigLoader, get_wallet_client, transaction_status_msg, transaction_submitted_msg
from greenbtc.cmds.units import units
from greenbtc.cmds.wallet_funcs import get_mojo_per_unit, get_wallet_type
...
from greenbtc.wallet.util.tx_config import DEFAULT_COIN_SELECTION_CONFIG
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        res = await wallet_client.create_new_dao_wallet(
            ...
            tx_config=CMDTXConfigLoader.from_json_dict({"reuse_puzhash": True}).to_tx_config(
                units["greenbtc"], config, fingerprint
            ),
            dao_rules=None,
            amount_of_cats=None,
...
async def create_dao_wallet(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    proposal_minimum = uint64(int(Decimal(args["proposal_minimum_amount"]) * units["greenbtc"]))
    ...
    final_fee_for_cat: uint64 = uint64(int(fee_for_cat * units["greenbtc"]))
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if len(conf_coins) < 2:  # pragma: no cover
            raise ValueError("DAO creation requires at least 2 gbtc coins in your wallet.")
        res = await wallet_client.create_new_dao_wallet(
            mode="new",
            dao_rules=dao_rules,
...
async def get_treasury_balance(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        gbtc_mojos = get_mojo_per_unit(WalletType.STANDARD_WALLET)
        cat_mojos = get_mojo_per_unit(WalletType.CAT)
        for asset_id, balance in balances.items():
            if asset_id == "gbtc":
                print(f"GBTC: {balance / gbtc_mojos}")
            else:
...
async def show_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, config):
        ...
        if ptype == "spend":
            gbtc_conds = pd["gbtc_conditions"]
            asset_conds = pd["asset_conditions"]
            print("")
            if gbtc_conds:
                print("Proposal GBTC Conditions")
                for pmt in gbtc_conds:
...
async def vote_on_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    fee = args["fee"]
    final_fee: uint64 = uint64(int(Decimal(fee) * units["greenbtc"]))
    proposal_id = args["proposal_id"]
...
async def create_spend_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if res["success"]:
            asset_id_name = asset_id if asset_id else "GBTC"
