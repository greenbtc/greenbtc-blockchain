...
from greenbtc.cmds.cmds_util import get_any_service_client
from greenbtc.cmds.start_funcs import async_start
from greenbtc.consensus.coinbase import create_puzzlehash_for_pk
from greenbtc.server.outbound_message import NodeType
from greenbtc.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
...
from greenbtc.util.config import load_config, save_config, set_peer_info
...
def create_greenbtc_directory(
    ...
    """
    This function creates a new greenbtc directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from greenbtc.cmds.init_funcs import greenbtc_init

    if not greenbtc_root.is_dir() or not Path(greenbtc_root / "config" / "config.yaml").exists():
        # create greenbtc directories & load config
        greenbtc_init(greenbtc_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(greenbtc_root, "config.yaml")
...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `greenbtc keys show`.
    """
...
def generate_and_return_fingerprint(mnemonic: Optional[str] = None) -> int:
    ...
    """
    from greenbtc.util.keychain import generate_mnemonic
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from greenbtc.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["GREENBTC_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create greenbtc directory & get config.
    print("Creating greenbtc directory & config...")
    config = create_greenbtc_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "greenbtc")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your GREENBTC_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from greenbtc.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically greenbtc show -s)
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['greenbtc']} TGBTC."
        )
        if show_addresses:
            print("All Addresses: ")
