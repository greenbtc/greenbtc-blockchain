...
from greenbtc.cmds.check_wallet_db import help_text as check_help_text
from greenbtc.cmds.coins import coins_cmd
from greenbtc.cmds.plotnft import validate_fee
...
@wallet_cmd.command("send", help="Send greenbtc to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in GBTC",
    type=str,
    default="0",
    show_default=True,
    required=True,
)
@click.option("-t", "--address", help="Address to send the GBTC", type=str, required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much GBTC or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much GBTC or CAT units",
    type=str,
    required=False,
    default="0",
)
...
@click.option(
    "-m", "--fee", help="A fee to add to the offer when it gets taken, in GBTC", default="0", show_default=True
)
def clawback(
...
@wallet_cmd.command("make_offer", help="Create an offer of GBTC/CATs/NFTs for GBTC/CATs/NFTs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m", "--fee", help="The fee to use when pushing the completed offer, in GBTC", default="0", show_default=True
)
@click.option(
    "--reuse",
    help="Reuse existing address for the offer.",
...
@click.option(
    "-m", "--fee", help="The fee to use when cancelling the offer securely, in GBTC", default="0", show_default=True
)
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck GreenBTC Wallet DBs"""

    from greenbtc.cmds.check_wallet_db import scan
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in GBTC.",
    type=str,
    default="0",
    show_default=True,
...
@click.option("-m", "--fee", help="The fee for the transaction, in GBTC", type=str)
...
@click.option("-m", "--fee", help="Blockchain fee for mint transaction, in GBTC", type=str, required=False, default="0")
def mint_vc_cmd(
    wallet_rpc_port: Optional[int],
    fingerprint: int,
    did: str,
...
@click.option(
    "-m", "--fee", help="Blockchain fee for update transaction, in GBTC", type=str, required=False, default="0"
)
@click.option(
...
@click.option(
    "-m", "--fee", help="Blockchain fee for revocation transaction, in GBTC", type=str, required=False, default="0"
)
@click.option(
    "--reuse-puzhash/--generate-new-puzhash",
    help="Send the VC back to the same puzzle hash it came from (ignored if --new-puzhash is specified)",
...
@wallet_cmd.command("find_pool_nft", short_help="fnd nft greenbtc")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-l",
    "--launcher-id",
    help="Set the launcher id, you should get it from greenbtc wallet",
    type=str,
    required=True
)
@click.option(
...
@wallet_cmd.command("recover_pool_nft", short_help="recover nft greenbtc to wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
