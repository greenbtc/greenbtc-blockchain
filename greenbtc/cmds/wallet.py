...
from greenbtc.cmds.cmds_util import execute_with_wallet
...
@wallet_cmd.command("send", short_help="Send greenbtc to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
def address_sign_message(wallet_rpc_port: Optional[int], fingerprint: int, address: str, hex_message: str) -> None:
    extra_params: Dict[str, Any] = {"address": address, "message": hex_message, "type": AddressType.GBTC}
    import asyncio
...
@wallet_cmd.command("make_offer", short_help="Create an offer of GBTC/CATs for GBTC/CATs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
