...
def print_transaction(tx: TransactionRecord, verbose: bool, name) -> None:
    if verbose:
        ...
        print(f"Amount {'sent' if tx.sent else 'received'}: {greenbtc_amount} {name}")
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    while time.time() - start < 10:
        ...
        if len(tx.sent_to) > 0:
            ...
            print(f"Do greenbtc wallet get_transaction -f {fingerprint} -tx 0x{tx_id} to get status")
            return None
...
async def send_from(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    withdraw_amount = uint64(int(request_amount * units["greenbtc"]))
    coin_records = None
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: int = None) -> Optional[Tuple[WalletRpcClient, int]]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'greenbtc keys generate' or import a key")
        return None
    ...
    if log_in_response["success"] is False:
        if log_in_response["error"] == "not_initialized":
            ...
            if "backup_path" in log_in_response:
                ...
                print(f"Backup file from backup.greenbtc.top downloaded and written to: {path}")
