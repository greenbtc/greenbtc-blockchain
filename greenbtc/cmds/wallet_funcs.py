...
from greenbtc.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg
...
from greenbtc.server.start_wallet import SERVICE_NAME
...
from greenbtc.util.config import load_config, selected_network_address_prefix
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(fee * units["greenbtc"]))
    final_amount: uint64
    final_min_coin_amount: uint64
    if typ == WalletType.STANDARD_WALLET:
        final_amount = uint64(int(amount * units["greenbtc"]))
        final_min_coin_amount = uint64(int(min_coin_amount * units["greenbtc"]))
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee: int = int(Decimal(args["fee"]) * units["greenbtc"])
        ...
        for item in [*offers, *requests]:
            ...
            if confirmation not in ["y", "yes"]:
                ...
                if offer is not None:
                    ...
                    print(f"Use greenbtc wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status")
                else:
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['greenbtc']}")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    from greenbtc.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    print(f"Included Fees: {Decimal(offer.bundle.fees()) / units['greenbtc']}")
...
async def mint_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    royalty_address = (
        ...
        else ensure_valid_address(args["royalty_address"], allowed_types={AddressType.GBTC}, config=config)
    )
    target_address = (
        None
        if not args["target_address"]
        else ensure_valid_address(args["target_address"], allowed_types={AddressType.GBTC}, config=config)
    )
...
async def transfer_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    try:
        ...
        nft_coin_id = args["nft_coin_id"]
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml")
        target_address = ensure_valid_address(args["target_address"], allowed_types={AddressType.GBTC}, config=config)
        fee: int = int(Decimal(args["fee"]) * units["greenbtc"])
...
async def send_notification(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    amount: uint64 = uint64(Decimal(args["amount"]) * units["greenbtc"])
    message: bytes = bytes(args["message"], "utf8")
    fee: uint64 = uint64(Decimal(args["fee"]) * units["greenbtc"])
