...
from greenbtc.cmds.wallet_funcs import print_balance, wallet_coin_unit
from greenbtc.pools.pool_config import load_pool_config, PoolWalletConfig, update_pool_config
from greenbtc.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
...
from greenbtc.util.bech32m import encode_puzzle_hash, decode_puzzle_hash
...
from greenbtc.cmds.cmds_util import transaction_submitted_msg, transaction_status_msg, get_any_service_client
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["greenbtc"]))
    ...
    if user_input.lower() == "y" or user_input.lower() == "yes":
        try:
            ...
            print(f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  greenbtc start -r farmer")
        return
    print("Aborting.")
