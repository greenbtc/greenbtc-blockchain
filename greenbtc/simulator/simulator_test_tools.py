...
from greenbtc.util.errors import KeychainFingerprintExists
...
def create_config(
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "tgbtc")
...
async def start_simulator(greenbtc_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    ...
    started_simulator = await start_simulator_main(True, automated_testing, root_path=greenbtc_root)
...
async def get_full_greenbtc_simulator(
    ...
    """
    A greenbtc root Path is required.
    The greenbtc root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    with Lockfile.create(daemon_launch_lock_path(greenbtc_root)):
        ...
        ws_server = WebSocketServer(greenbtc_root, ca_crt_path, ca_key_path, crt_path, key_path)
        async with SignalHandlers.manage() as signal_handlers:
            ...
            async with ws_server.run():
                async for simulator in start_simulator(greenbtc_root, automated_testing):
                    yield simulator, greenbtc_root, config, mnemonic, fingerprint, keychain
