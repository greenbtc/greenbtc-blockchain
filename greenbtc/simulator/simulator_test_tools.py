...
from greenbtc.cmds.init_funcs import create_all_ssl
...
def create_config(
    ...
    # create greenbtc directories
    create_default_greenbtc_config(greenbtc_root)
    create_all_ssl(
        greenbtc_root,
    ...
    # load config
    config = load_config(greenbtc_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(fingerprint), "tgbtc")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(greenbtc_root, "config.yaml", config)
    return config


async def start_simulator(greenbtc_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=greenbtc_root)
    await service.start()
...
async def get_full_greenbtc_simulator(
    automated_testing: bool = False, greenbtc_root: Optional[Path] = None, config: Optional[Dict[str, Any]] = None
) -> AsyncGenerator[Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int], None]:
    """
    A greenbtc root directory can be provided, otherwise a temporary one is created.
    This test can either be run in automated mode or not, which determines which mode block tools run in.
    This test is fully interdependent and can be used without the rest of the greenbtc test suite.
    Please refer to the documentation for more information.
    """
    # Create and setup temporary greenbtc directories.
    if greenbtc_root is None:
        greenbtc_root = Path(tempfile.TemporaryDirectory().name)
    ...
    crt_path = greenbtc_root / config["daemon_ssl"]["private_crt"]
    key_path = greenbtc_root / config["daemon_ssl"]["private_key"]
    ca_crt_path = greenbtc_root / config["private_ssl_ca"]["crt"]
    ca_key_path = greenbtc_root / config["private_ssl_ca"]["key"]
    with Lockfile.create(daemon_launch_lock_path(greenbtc_root)):
        shutdown_event = asyncio.Event()
        ws_server = WebSocketServer(greenbtc_root, ca_crt_path, ca_key_path, crt_path, key_path, shutdown_event)
        await ws_server.setup_process_global_state()
        await ws_server.start()  # type: ignore[no-untyped-call]

        async for simulator in start_simulator(greenbtc_root, automated_testing):
            yield simulator, greenbtc_root, config, mnemonic, fingerprint
