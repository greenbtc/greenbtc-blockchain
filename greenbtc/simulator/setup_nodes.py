...
from greenbtc.daemon.server import WebSocketServer
...
from greenbtc.util.ints import uint16, uint32
...
SimulatorsAndWallets = Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, GreenBTCServer]], BlockTools]
...
) -> AsyncGenerator[Tuple[FullNodeAPI, FullNodeAPI, GreenBTCServer, GreenBTCServer, BlockTools], None]:
    """
...
) -> AsyncGenerator[Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, GreenBTCServer]], BlockTools], None]:
    with TempKeyring(populate=True) as keychain1, TempKeyring(populate=True) as keychain2:
...
async def setup_full_system(
    ...
    Tuple[Any, Any, Harvester, Farmer, Any, Service[Timelord, TimelordAPI], object, object, Any, GreenBTCServer], None
]:
    with TempKeyring(populate=True) as keychain1, TempKeyring(populate=True) as keychain2:
...
async def setup_full_system_inner(
    ...
    Tuple[Any, Any, Harvester, Farmer, Any, Service[Timelord, TimelordAPI], object, object, Any, GreenBTCServer],
]:
    if b_tools is None:
