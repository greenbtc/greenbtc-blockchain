...
from greenbtc.cmds.init_funcs import init
from greenbtc.consensus.constants import ConsensusConstants
from greenbtc.daemon.server import WebSocketServer, daemon_launch_lock_path
from greenbtc.farmer.farmer import Farmer
from greenbtc.farmer.farmer_api import FarmerAPI
...
from greenbtc.introducer.introducer_api import IntroducerAPI
from greenbtc.protocols.shared_protocol import Capability, capabilities
from greenbtc.seeder.crawler import Crawler
from greenbtc.seeder.crawler_api import CrawlerAPI
from greenbtc.seeder.dns_server import DNSServer, create_dns_server_service
from greenbtc.seeder.start_crawler import create_full_node_crawler_service
from greenbtc.server.outbound_message import NodeType
from greenbtc.server.start_farmer import create_farmer_service
from greenbtc.server.start_full_node import create_full_node_service
from greenbtc.server.start_harvester import create_harvester_service
from greenbtc.server.start_introducer import create_introducer_service
from greenbtc.server.start_service import Service
from greenbtc.server.start_timelord import create_timelord_service
from greenbtc.server.start_wallet import create_wallet_service
from greenbtc.simulator.block_tools import BlockTools, test_constants
from greenbtc.simulator.full_node_simulator import FullNodeSimulator
from greenbtc.simulator.keyring import TempKeyring
from greenbtc.simulator.ssl_certs import get_next_nodes_certs_and_keys, get_next_private_ca_cert_and_key
from greenbtc.simulator.start_simulator import create_full_node_simulator_service
from greenbtc.ssl.create_ssl import create_all_ssl
from greenbtc.timelord.timelord import Timelord
from greenbtc.timelord.timelord_api import TimelordAPI
from greenbtc.timelord.timelord_launcher import VDFClientProcessMgr, find_vdf_client, spawn_process
from greenbtc.types.blockchain_format.sized_bytes import bytes32
from greenbtc.types.peer_info import UnresolvedPeerInfo
from greenbtc.util.bech32m import encode_puzzle_hash
from greenbtc.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config, set_peer_info
from greenbtc.util.db_wrapper import generate_in_memory_db_uri
from greenbtc.util.ints import uint16
from greenbtc.util.keychain import bytes_to_mnemonic
from greenbtc.util.lock import Lockfile
from greenbtc.util.misc import SignalHandlers
from greenbtc.wallet.wallet_node import WalletNode
from greenbtc.wallet.wallet_node_api import WalletNodeAPI
...
async def setup_wallet_node(
    ...
    with TempKeyring(populate=True) as keychain:
        ...
        service_config["gbtc_spam_amount"] = gbtc_spam_amount
...
async def setup_farmer(
    ...
    service_config["gbtc_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "gbtc")
    service_config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    service_config["port"] = port
    service_config["rpc_port"] = uint16(0)
    config_pool["gbtc_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "gbtc")
