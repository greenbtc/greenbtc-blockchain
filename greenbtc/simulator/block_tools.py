...
from greenbtc.consensus.block_creation import create_unfinished_block, unfinished_block_to_full_block
...
from greenbtc.types.blockchain_format.program import INFINITE_COST, Program
...
from greenbtc.util.ssl_check import fix_ssl
...
from greenbtc.wallet.puzzles.load_clvm import load_serialized_clvm_maybe_recompile

GENERATOR_MOD: SerializedProgram = load_serialized_clvm_maybe_recompile(
    "rom_bootstrap_generator.clsp", package_or_requirement="greenbtc.consensus.puzzles"
)
...
class BlockTools:
        ...
        while True:
            ...
            if num_empty_slots_added < skip_slots:
                ...
                for signage_point_index in range(0, constants.NUM_SPS_SUB_SLOT - constants.NUM_SP_INTERVALS_EXTRA):
                    ...
                    qualified_proofs: List[Tuple[uint64, ProofOfSpace, ProofOfStake]] = self.get_pospaces_for_challenge(
                        constants,
                        slot_cc_challenge,
                        cc_sp_output_hash,
                        seed,
                    ...
                    for required_iters, proof_of_space, proof_of_stake in sorted(qualified_proofs, key=lambda t: t[0]):
    ...
    def create_genesis_block(
        ...
        while True:
            ...
            for signage_point_index in range(0, constants.NUM_SPS_SUB_SLOT):
                ...
                for required_iters, proof_of_space, proof_of_stake in qualified_proofs:
    ...
    def get_pospaces_for_challenge(
        ...
        height: uint32,
        force_plot_id: Optional[bytes32] = None,
    ) -> List[Tuple[uint64, ProofOfSpace, ProofOfStake]]:
        found_proofs: List[Tuple[uint64, ProofOfSpace, ProofOfStake]] = []
        rng = random.Random()
        rng.seed(seed)
        for plot_info in self.plot_manager.plots.values():
            ...
            if passes_plot_filter(prefix_bits, plot_id, challenge_hash, signage_point):
                ...
                stake = ProofOfStake(0, uint64(STAKE_PER_COEFFICIENT))
                for proof_index, quality_str in enumerate(qualities):
                    ...
                    if required_iters < calculate_sp_interval_iters(constants, sub_slot_iters):
                        ...
                        found_proofs.append((required_iters, proof_of_space, stake))
...
def load_block_list(
    ...
    for full_block in block_list:
        ...
        required_iters: uint64 = calculate_iterations_quality(
            ...
            uint64(difficulty),
            sp_hash,
            full_block.proof_of_stake.coefficient,
        )
