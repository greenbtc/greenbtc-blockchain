...
from greenbtc.cmds.init_funcs import create_all_ssl, create_default_greenbtc_config
...
from greenbtc.consensus.coinbase import create_farmer_coin, create_pool_coin, create_puzzlehash_for_pk
from greenbtc.consensus.condition_costs import ConditionCost
...
from greenbtc.consensus.full_block_to_block_record import block_to_block_record
from greenbtc.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from greenbtc.consensus.pot_iterations import (
    calculate_ip_iters,
...
from greenbtc.consensus.vdf_info_computation import get_signage_point_vdf_info
...
from greenbtc.full_node.generator import setup_generator_args
from greenbtc.full_node.mempool_check_conditions import GENERATOR_MOD
from greenbtc.full_node.signage_point import SignagePoint
from greenbtc.plotting.create_plots import PlotKeys, create_plots
...
from greenbtc.server.server import ssl_context_for_client
from greenbtc.simulator.socket import find_available_listen_port
from greenbtc.simulator.ssl_certs import (
...
from greenbtc.simulator.time_out_assert import time_out_assert_custom_interval
from greenbtc.simulator.wallet_tools import WalletTool
...
from greenbtc.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from greenbtc.types.blockchain_format.pool_target import PoolTarget
from greenbtc.types.blockchain_format.program import INFINITE_COST
...
from greenbtc.types.blockchain_format.slots import (
...
from greenbtc.types.generator_types import BlockGenerator, CompressorArg
...
from greenbtc.util.block_cache import BlockCache
from greenbtc.util.config import config_path_for_filename, load_config, lock_config, override_config, save_config
...
from greenbtc.util.keychain import Keychain, bytes_to_mnemonic
from greenbtc.util.prev_transaction_block import get_prev_transaction_block
from greenbtc.util.vdf_prover import get_vdf_info_and_proof
    ...
    async def setup_keys(self, fingerprint: Optional[int] = None, reward_ph: Optional[bytes32] = None):
        if self.local_keychain:
            ...
            keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, self.log, user="testing-1.8.0", service="greenbtc-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `greenbtc keys generate`")
        ...
        try:
            if plot_keys is None:
                ...
                if pool_contract_puzzle_hash is None:
                    ...
                    pool_address = encode_puzzle_hash(pool_contract_puzzle_hash, "gbtc")
