...
from greenbtc.consensus.block_creation import unfinished_block_to_full_block
from greenbtc.consensus.block_record import BlockRecord
from greenbtc.consensus.blockchain import AddBlockResult, Blockchain, BlockchainMutexPriority, StateChangeSummary
...
from greenbtc.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from greenbtc.consensus.multiprocess_validation import PreValidationResult
from greenbtc.consensus.pot_iterations import calculate_sp_iters
from greenbtc.full_node.block_store import BlockStore
from greenbtc.full_node.bundle_tools import detect_potential_template_generator
from greenbtc.full_node.coin_store import CoinStore
from greenbtc.full_node.full_node_api import FullNodeAPI
from greenbtc.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult
from greenbtc.full_node.hint_management import get_hints_and_subscription_coin_ids
from greenbtc.full_node.hint_store import HintStore
from greenbtc.full_node.mempool_manager import MempoolManager
from greenbtc.full_node.signage_point import SignagePoint
from greenbtc.full_node.subscriptions import PeerSubscriptions
from greenbtc.full_node.sync_store import SyncStore
from greenbtc.full_node.tx_processing_queue import TransactionQueue
from greenbtc.full_node.weight_proof import WeightProofHandler
from greenbtc.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from greenbtc.protocols.full_node_protocol import RequestBlocks, RespondBlock, RespondBlocks, RespondSignagePoint
from greenbtc.protocols.protocol_message_types import ProtocolMessageTypes
from greenbtc.protocols.wallet_protocol import CoinState, CoinStateUpdate
from greenbtc.rpc.rpc_server import StateChangedProtocol
from greenbtc.server.node_discovery import FullNodePeers
from greenbtc.server.outbound_message import Message, NodeType, make_msg
...
from greenbtc.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from greenbtc.types.transaction_queue_entry import TransactionQueueEntry
from greenbtc.types.unfinished_block import UnfinishedBlock
from greenbtc.util import cached_bls
from greenbtc.util.bech32m import encode_puzzle_hash
from greenbtc.util.check_fork_next_block import check_fork_next_block
from greenbtc.util.condition_tools import pkm_pairs
from greenbtc.util.config import PEER_DB_PATH_KEY_DEPRECATED, process_config_start_method
from greenbtc.util.db_synchronous import db_synchronous_on
from greenbtc.util.db_version import lookup_db_version, set_db_version_async
from greenbtc.util.db_wrapper import DBWrapper2, manage_connection
from greenbtc.util.errors import ConsensusError, Err, ValidationError
from greenbtc.util.ints import uint8, uint32, uint64, uint128
from greenbtc.util.limited_semaphore import LimitedSemaphore
from greenbtc.util.path import path_from_root
from greenbtc.util.profiler import mem_profile_task, profile_task
from greenbtc.util.safe_cancel_task import cancel_task_safe
...
class FullNode:
                ...
                async with conn.execute(
                    ...
                    if len(list(await cur.fetchall())) == 0:
                        try:
                            ...
                            # it could be a database created with "greenbtc init", which is
    ...
    def set_server(self, server: GreenBTCServer) -> None:
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.greenbtc.top")
        try:
    ...
    async def short_sync_batch(self, peer: WSGreenBTCConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: WSGreenBTCConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ) -> bool:
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: WSGreenBTCConnection) -> None:
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[WSGreenBTCConnection] = None
    ) -> None:
        """
        ...
        if peak_block is not None:
            ...
            difficulty_coefficient = await self.blockchain.get_staking_coefficient(
                peak_block.height - 1 if peak.height > 0 else 0, peak.farmer_public_key
            )
    ...
    async def on_connect(self, connection: WSGreenBTCConnection) -> None:
        """
    ...
    def on_disconnect(self, connection: WSGreenBTCConnection) -> None:
    ...
    async def _sync(self) -> None:
        ...
        try:
            ...
            weight_proof_peer: WSGreenBTCConnection = random.choice(peers_with_peak)
            self.log.info(
    ...
    async def sync_from_fork_point(
        ...
        peers_with_peak: List[WSGreenBTCConnection] = self.get_peers_with_peak(peak_hash)
        ...
        async def fetch_block_batches(
            batch_queue: asyncio.Queue[Optional[Tuple[WSGreenBTCConnection, List[FullBlock]]]]
        ) -> None:
            start_height, end_height = 0, 0
            new_peers_with_peak: List[WSGreenBTCConnection] = peers_with_peak[:]
            try:
        ...
        async def validate_block_batches(
            inner_batch_queue: asyncio.Queue[Optional[Tuple[WSGreenBTCConnection, List[FullBlock]]]]
        ) -> None:
            advanced_peak: bool = False
            while True:
                res: Optional[Tuple[WSGreenBTCConnection, List[FullBlock]]] = await inner_batch_queue.get()
                if res is None:
        ...
        batch_queue_input: asyncio.Queue[Optional[Tuple[WSGreenBTCConnection, List[FullBlock]]]] = asyncio.Queue(
            maxsize=buffer_size
        )
    ...
    def get_peers_with_peak(self, peak_hash: bytes32) -> List[WSGreenBTCConnection]:
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: WSGreenBTCConnection = self.server.all_connections[peer]
    ...
    async def add_block_batch(
        ...
        all_validate_start = time.monotonic()
        ...
        for block in blocks_to_validate:
            ...
            validation_results: List[PreValidationResult] = await self.blockchain.pre_validate_blocks_multiprocessing(
                [block], [difficulty_coefficient], {},
                wp_summaries=wp_summaries, validate_signatures=True,
            )

            if validation_results[0].error is not None:
                self.log.error(
                    f"Invalid block from peer: {peer.get_peer_logging()} {Err(validation_results[0].error)}"
                )
                return False, None

            assert validation_results[0].required_iters is not None
            state_change_summary: Optional[StateChangeSummary]
            advanced_peak = agg_state_change_summary is not None
            result, error, state_change_summary = await self.blockchain.add_block(
                block, validation_results[0], None if advanced_peak else fork_point
            )
            ...
            validate_time = time.monotonic() - validate_start

            self.log.log(
                logging.WARNING if validate_time > 10 else logging.DEBUG,
                f"height {block.height} Block validation time: {validate_time:0.2f} seconds "
            )

        if agg_state_change_summary is not None:
            self._state_changed("new_peak")
            self.log.debug(
                f"Total time for {len(blocks_to_validate)} blocks: {time.monotonic() - all_validate_start:0.2f}, "
                f"advanced: True"
            )
    ...
    async def add_block(
        self,
        block: FullBlock,
        peer: Optional[WSGreenBTCConnection] = None,
        ...
        async with self.blockchain.priority_mutex.acquire(priority=BlockchainMutexPriority.high):
            ...
            pre_validation_results = await self.blockchain.pre_validate_blocks_multiprocessing(
                [block], [difficulty_coefficient], npc_results, validate_signatures=False
            )
    ...
    async def add_unfinished_block(
        ...
        difficulty_coefficient = await self.blockchain.get_staking_coefficient(
            uint32(0) if prev_b is None else prev_b.height,
            block.reward_chain_block.proof_of_space.farmer_public_key,
        )

        async with self.blockchain.priority_mutex.acquire(priority=BlockchainMutexPriority.high):
            start_header_time = time.time()
            _,  header_error = await self.blockchain.validate_unfinished_block_header(block, difficulty_coefficient)
        ...
        async with self.blockchain.priority_mutex.acquire(priority=BlockchainMutexPriority.high):
            ...
            validation_start = time.time()
            validate_result = await self.blockchain.validate_unfinished_block(block, npc_result, difficulty_coefficient)
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'gbtc')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[WSGreenBTCConnection] = None
    ) -> Optional[Message]:
    ...
    async def add_end_of_sub_slot(
        self, end_of_slot_bundle: EndOfSubSlotBundle, peer: WSGreenBTCConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: WSGreenBTCConnection) -> None:
        peak = self.blockchain.get_peak()
        if peak is None or peak.height - request.height < 5:
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: WSGreenBTCConnection) -> None:
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def add_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSGreenBTCConnection) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: WSGreenBTCConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
    block_response: Optional[Any] = await peer.call_api(
