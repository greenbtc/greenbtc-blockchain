...
from greenbtc.consensus.block_creation import create_unfinished_block
from greenbtc.consensus.block_record import BlockRecord
from greenbtc.consensus.blockchain import BlockchainMutexPriority
from greenbtc.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from greenbtc.full_node.bundle_tools import (
    best_solution_generator_from_template,
    simple_solution_generator,
    simple_solution_generator_backrefs,
)
from greenbtc.full_node.fee_estimate import FeeEstimate, FeeEstimateGroup, fee_rate_v2_to_v1
from greenbtc.full_node.fee_estimator_interface import FeeEstimatorInterface
from greenbtc.full_node.mempool_check_conditions import get_name_puzzle_conditions, get_puzzle_and_solution_for_coin
from greenbtc.full_node.signage_point import SignagePoint
from greenbtc.full_node.tx_processing_queue import TransactionQueueFull
from greenbtc.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from greenbtc.protocols.full_node_protocol import RejectBlock, RejectBlocks
...
from greenbtc.types.block_protocol import BlockInfo
from greenbtc.types.blockchain_format.coin import Coin, hash_coin_ids
...
from greenbtc.util.full_block_utils import header_block_from_block
from greenbtc.util.generator_tools import get_block_header, tx_removals_and_additions
from greenbtc.util.hash import std_hash
from greenbtc.util.ints import uint8, uint32, uint64, uint128
from greenbtc.util.limited_semaphore import LimitedSemaphoreFullError
from greenbtc.util.merkle_set import MerkleSet

if TYPE_CHECKING:
    from greenbtc.full_node.full_node import FullNode
else:
    FullNode = object
...
class FullNodeAPI:
    ...
    async def request_peers(
        self, _request: full_node_protocol.RequestPeers, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: WSGreenBTCConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: WSGreenBTCConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def request_block_header(self, request: wallet_protocol.RequestBlockHeader) -> Optional[Message]:
        ...
        msg = make_msg(
            ...
            wallet_protocol.RespondBlockHeader(header_block, difficulty_coefficient),
        )
        return msg
    ...
    async def new_compact_vdf(
        self, request: full_node_protocol.NewCompactVDF, peer: WSGreenBTCConnection, request_bytes: bytes = b""
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @api_request(peer_required=True)
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSGreenBTCConnection) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def register_interest_in_puzzle_hash(
        self, request: wallet_protocol.RegisterForPhUpdates, peer: WSGreenBTCConnection
    ) -> Message:
    ...
    async def register_interest_in_coin(
        self, request: wallet_protocol.RegisterForCoinUpdates, peer: WSGreenBTCConnection
    ) -> Message:
    ...
    # staking
    @api_request()
    async def request_stakings(
        self, request: farmer_protocol.RequestStakings
    ) -> Optional[Message]:
        staking_coefficients: List[Tuple[G1Element, str]] = []
        for pk in request.public_keys:
            coefficient = await self.full_node.blockchain.get_staking_coefficient(request.height, pk)
            staking_coefficients.append((pk, str(coefficient)))
        return make_msg(
            ProtocolMessageTypes.respond_stakings,
            farmer_protocol.FarmerStakings(staking_coefficients),
        )
