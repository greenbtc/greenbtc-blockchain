...
from greenbtc.protocols import full_node_protocol
from greenbtc.protocols.full_node_protocol import RespondPeers
from greenbtc.rpc.rpc_server import StateChangedProtocol, default_get_connections
from greenbtc.seeder.crawl_store import CrawlStore
from greenbtc.seeder.peer_record import PeerRecord, PeerReliability
...
class Crawler:
    ...
    async def create_client(
        self, peer_info: PeerInfo, on_connect: Callable[[WSGreenBTCConnection], Awaitable[None]]
    ) -> bool:
    ...
    async def connect_task(self, peer: PeerRecord) -> None:
        ...
        async def peer_action(peer: WSGreenBTCConnection) -> None:
