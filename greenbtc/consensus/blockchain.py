...
from greenbtc.consensus.block_header_validation import validate_finished_header_block, validate_unfinished_header_block
...
from greenbtc.types.generator_types import BlockGenerator, GeneratorArg
...
from greenbtc.util.streamable import recurse_jsonify
...
class Blockchain(BlockchainInterface):
    ...
    async def validate_unfinished_block(
        ...
        return PreValidationResult(None, required_iters, cost_result, str(difficulty_coefficient))
    ...
    async def get_farmer_difficulty_coefficient(
        ...
        if minimal_staking != 0 and staking >= minimal_staking:
            if blocks == 0 or network_space == 0:
                ...
                coeff = Decimal("0.2") + Decimal(1) / (Decimal(staking) / space / 2 + Decimal("0.25"))
            else:
                coeff = Decimal("0.05") + Decimal(1) / (Decimal(staking) / space / 2 + Decimal("0.05"))
