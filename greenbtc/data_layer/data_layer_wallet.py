...
from greenbtc.data_layer.data_layer_errors import LauncherCoinNotFoundError, OfferIntegrityError
from greenbtc.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from greenbtc.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from greenbtc.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from greenbtc.wallet.trading.offer import NotarizedPayment, Offer
...
from greenbtc.wallet.util.merkle_utils import _simplify_merkle_proof
from greenbtc.wallet.util.transaction_type import TransactionType
from greenbtc.wallet.util.tx_config import CoinSelectionConfig, TXConfig, TXConfigLoader
from greenbtc.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSGreenBTCConnection) -> CoinState:
    ...
    async def create_update_state_spend(
        ...
        if fee > 0:
            greenbtc_tx = await self.create_tandem_gbtc_tx(
                fee, Announcement(current_coin.name(), b"$"), tx_config, coin_announcement=True
            )
            assert greenbtc_tx.spend_bundle is not None
            aggregate_bundle = SpendBundle.aggregate([dl_tx.spend_bundle, greenbtc_tx.spend_bundle])
            ...
            txs: List[TransactionRecord] = [dl_tx, greenbtc_tx]
        else:
            txs = [dl_tx]
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            [greenbtc_tx] = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
                tx_config,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, greenbtc_tx.spend_bundle])
                ),
                dataclasses.replace(greenbtc_tx, spend_bundle=None),
            ]
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSGreenBTCConnection, coin_data: Optional[object]) -> None:
