...
from greenbtc.plot_sync.delta import Delta
from greenbtc.plot_sync.receiver import Receiver
from greenbtc.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from greenbtc.server.server import GreenBTCServer, ssl_context_for_root
...
from greenbtc.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from greenbtc.util.errors import KeychainProxyConnectionFailure
...
from greenbtc.util.logging import TimedDuplicateFilter
...
from greenbtc.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    def __init__(
        ...
        self.proofs_of_space: Dict[bytes32, List[Tuple[str, ProofOfSpace, ProofOfStake]]] = {}
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'greenbtc keys generate' or open the UI."
        try:
        ...
        if "gbtc_target_address" not in self.config:
            self.config = config["farmer"]
        if "gbtc_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "gbtc_target_address" not in self.config or "gbtc_target_address" not in self.pool_config:
            log.debug("gbtc_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["gbtc_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["gbtc_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]) -> None:
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["gbtc_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["gbtc_target_address"] = pool_target_encoded
