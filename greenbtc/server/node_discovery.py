...
from greenbtc.protocols.full_node_protocol import RequestPeers, RespondPeers
from greenbtc.protocols.introducer_protocol import RequestPeersIntroducer
...
from greenbtc.types.peer_info import PeerInfo, TimestampedPeerInfo, UnresolvedPeerInfo
from greenbtc.util.hash import std_hash
from greenbtc.util.ints import uint16, uint64
from greenbtc.util.network import IPAddress, resolve
...
class FullNodeDiscovery:
    ...
    async def update_peer_timestamp_on_message(self, peer: WSGreenBTCConnection) -> None:
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self) -> None:
        ...
        async def on_connect(peer: WSGreenBTCConnection) -> None:
    ...
    async def on_connect_callback(self, peer: WSGreenBTCConnection) -> None:
