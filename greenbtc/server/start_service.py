...
from greenbtc.daemon.server import service_launch_lock_path
from greenbtc.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from greenbtc.server.api_protocol import ApiProtocol
from greenbtc.server.greenbtc_policy import set_greenbtc_policy
from greenbtc.server.outbound_message import NodeType
from greenbtc.server.server import GreenBTCServer
from greenbtc.server.ssl_context import greenbtc_ssl_ca_paths, private_ssl_ca_paths
from greenbtc.server.upnp import UPnP
from greenbtc.server.ws_connection import WSGreenBTCConnection
from greenbtc.types.peer_info import PeerInfo, UnresolvedPeerInfo
...
from greenbtc.util.network import resolve
...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSGreenBTCConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"greenbtc-blockchain version: {greenbtc_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        greenbtc_ca_crt, greenbtc_ca_key = greenbtc_ssl_ca_paths(root_path, self.config)
        ...
        self._server = GreenBTCServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
            ...
            (greenbtc_ca_crt, greenbtc_ca_key),
    ...
    async def manage(self, *, start: bool = True) -> AsyncIterator[None]:
        ...
        async with contextlib.AsyncExitStack() as async_exit_stack:
            try:
                ...
                self._log.info("Waiting for GreenBTCServer to be closed")
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"greenbtc_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    with log_exceptions(log=log, message="fatal uncaught exception"):
        if connection_limit is not None:
            set_greenbtc_policy(connection_limit)
