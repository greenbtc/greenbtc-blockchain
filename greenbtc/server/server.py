...
from greenbtc.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
...
from greenbtc.util.network import is_localhost, is_in_network
...
class ChiaServer:
    def __init__(
        ...
        self.greenbtc_ca_crt_path, self.greenbtc_ca_key_path = greenbtc_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.greenbtc_ca_crt_path, self.greenbtc_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.greenbtc_ca_crt_path, self.greenbtc_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSChiaConnection] = None
        try:
