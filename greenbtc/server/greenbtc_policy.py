...
if TYPE_CHECKING:
    ...
    class BaseEventsServer(asyncio.base_events.Server):
        if sys.platform == "win32":
            _loop: GreenBTCProactorEventLoop
        else:
...
class PausableServer(BaseEventsServer):
    ...
    def _detach(self) -> None:
        ...
        if (
            ...
            self._greenbtc_resume()

    def _greenbtc_pause(self) -> None:
    ...
    def _greenbtc_resume(self) -> None:
...
async def _greenbtc_create_server(
    cls: Any,
    protocol_factory: _ProtocolFactory,
    host: Any,
    port: Any,
...
class GreenBTCSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _greenbtc_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class GreenBTCProactor(IocpProactor):
        ...
        async def _greenbtc_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._greenbtc_accept(listener)
                except WindowsError as exc:  # pylint: disable=E0602
                    if exc.winerror not in (_winapi.ERROR_NETNAME_DELETED, _winapi.ERROR_OPERATION_ABORTED):
                        raise

        def _greenbtc_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: GreenBTCProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._greenbtc_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class GreenBTCProactorEventLoop(ProactorEventLoop):
...
class GreenBTCPolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        # overriding https://github.com/python/cpython/blob/v3.11.0/Lib/asyncio/events.py#L689-L695
        if sys.platform == "win32":
            loop_factory = GreenBTCProactorEventLoop
        else:
            loop_factory = GreenBTCSelectorEventLoop

        return loop_factory()


def set_greenbtc_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(GreenBTCPolicy())
