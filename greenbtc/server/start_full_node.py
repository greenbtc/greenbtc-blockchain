...
from greenbtc.consensus.default_constants import DEFAULT_CONSTANTS, update_testnet_overrides
from greenbtc.full_node.full_node import FullNode
from greenbtc.full_node.full_node_api import FullNodeAPI
from greenbtc.rpc.full_node_rpc_api import FullNodeRpcApi
...
def main() -> int:
    ...
    with maybe_manage_task_instrumentation(enable=os.environ.get("GREENBTC_INSTRUMENT_NODE") is not None):
        ...
        if not service_config.get("use_greenbtc_loop_policy", True):
            target_peer_count = None
