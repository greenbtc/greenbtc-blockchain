...
from greenbtc.cmds.init_funcs import check_keys, greenbtc_full_version_str, greenbtc_init
...
from greenbtc.plotters.plotters import get_available_plotters
from greenbtc.plotting.util import add_plot_directory
from greenbtc.server.server import ssl_context_for_server
...
from greenbtc.util.keychain import Keychain, KeyData, passphrase_requirements, supports_os_passphrase_storage
...
service_plotter = "greenbtc_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "greenbtc_full_node_simulator": "greenbtc_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["greenbtc", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'greenbtc plotters bladebit ramplot' or 'greenbtc plotters bladebit diskplot'
            plot_type = request["plot_type"]
