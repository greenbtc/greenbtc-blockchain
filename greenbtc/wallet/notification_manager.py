...
from greenbtc.wallet.notification_store import Notification, NotificationStore
from greenbtc.wallet.transaction_record import TransactionRecord
from greenbtc.wallet.util.compute_memos import compute_memos_for_spend
from greenbtc.wallet.util.notifications import construct_notification
...
class NotificationManager:
    ...
    async def send_new_notification(
        ...
        full_tx: TransactionRecord = dataclasses.replace(
            greenbtc_tx, spend_bundle=SpendBundle.aggregate([greenbtc_tx.spend_bundle, extra_spend_bundle])
        )
        return full_tx
