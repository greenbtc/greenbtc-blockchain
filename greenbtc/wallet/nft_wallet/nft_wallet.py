...
import greenbtc.wallet.singleton
...
from greenbtc.wallet.conditions import Condition, UnknownCondition, parse_timelock_info
from greenbtc.wallet.derivation_record import DerivationRecord
from greenbtc.wallet.did_wallet import did_wallet_puzzles
from greenbtc.wallet.did_wallet.did_info import DIDInfo
from greenbtc.wallet.lineage_proof import LineageProof
from greenbtc.wallet.nft_wallet import nft_puzzles
from greenbtc.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from greenbtc.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from greenbtc.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from greenbtc.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
...
from greenbtc.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
...
from greenbtc.wallet.util.tx_config import CoinSelectionConfig, TXConfig
...
from greenbtc.wallet.wallet_protocol import GSTOptionalArgs, WalletProtocol
...
class NFTWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSGreenBTCConnection, parent_coin_data: Optional[NFTCoinData]
    ) -> None:
    ...
    async def puzzle_solution_received(self, coin: Coin, data: NFTCoinData, peer: WSGreenBTCConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        unsigned_spend_bundle, greenbtc_tx = await self.generate_unsigned_spendbundle(
            payments,
            tx_config,
            fee,
            coins=coins,
        ...
        if greenbtc_tx is not None and greenbtc_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, greenbtc_tx.spend_bundle])
            greenbtc_tx = dataclasses.replace(greenbtc_tx, spend_bundle=None)
        ...
        if greenbtc_tx is not None:
            tx_list.append(greenbtc_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            greenbtc_tx = await self.standard_wallet.create_tandem_gbtc_tx(
        ...
        return nft_spend_bundle, greenbtc_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'GBTC'} to offer")
    ...
    async def mint_from_did(
        ...
        gbtc_coins: Optional[Set[Coin]] = None,
        gbtc_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an gbtc spend that funds the transactions and pays fees.
        ...
        :param gbtc_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param gbtc_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the gbtc spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an gbtc coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if gbtc_coins is None:
            gbtc_coins = await self.standard_wallet.select_coins(uint64(total_amount), tx_config.coin_selection_config)
        assert len(gbtc_coins) > 0
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            intermediate_launcher_puz = did_wallet_puzzles.INTERMEDIATE_LAUNCHER_MOD.curry(
                greenbtc.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, mint_number, mint_total
            )
            ...
            launcher_coin = Coin(
                intermediate_launcher_coin.name(), greenbtc.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, amount
            )
            ...
            launcher_cs = CoinSpend(
                launcher_coin, greenbtc.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE, genesis_launcher_solution
            )
        ...
        # Create the gbtc spend to fund the minting.
        spend_value = sum([coin.amount for coin in gbtc_coins])
        change: uint64 = uint64(spend_value - total_amount)
        gbtc_spends = []
        if gbtc_change_ph is None:
            gbtc_change_ph = await self.standard_wallet.get_new_puzzlehash()
        gbtc_payment = Payment(gbtc_change_ph, change, [gbtc_change_ph])

        first = True
        for gbtc_coin in gbtc_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(gbtc_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in gbtc_coins]
                message_list.append(Coin(gbtc_coin.name(), gbtc_payment.puzzle_hash, gbtc_payment.amount).name())
                message: bytes32 = std_hash(b"".join(message_list))

                if len(gbtc_coins) > 1:
                    gbtc_announcement: Optional[Set[bytes]] = {message}
                else:
                    gbtc_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=[gbtc_payment],
                    fee=fee,
                    coin_announcements=gbtc_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(gbtc_coin.name(), message).name()
            ...
            gbtc_spends.append(CoinSpend(gbtc_coin, puzzle, solution))
        gbtc_spend = await self.wallet_state_manager.sign_transaction(gbtc_spends)
        ...
        did_full_puzzle: Program = greenbtc.wallet.singleton.create_singleton_puzzle(
            innerpuz,
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, gbtc_spend, *eve_spends])

        tx_record: TransactionRecord = dataclasses.replace(eve_txs[0], spend_bundle=total_spend)
        return [tx_record]

    async def mint_from_gbtc(
        self,
        metadata_list: List[Dict[str, Any]],
        tx_config: TXConfig,
        target_list: Optional[List[bytes32]] = [],
        ...
        """
        Minting NFTs from a single GBTC spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = gbtc_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from GBTC to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the GBTC spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the GBTC spend
            ...
            # Annnouncements for eve spend. These are asserted by the gbtc spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for gbtc_coin in gbtc_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=[gbtc_payment] + primaries,
                    fee=fee,
                    coin_announcements=gbtc_announcement if len(gbtc_coins) > 1 else None,
