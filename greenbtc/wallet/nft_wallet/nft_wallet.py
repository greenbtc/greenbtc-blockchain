...
from greenbtc.util.ints import uint8, uint16, uint32, uint64, uint128
...
from greenbtc.wallet.util.debug_spend_bundle import disassemble
from greenbtc.wallet.util.transaction_type import TransactionType
from greenbtc.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
class NFTWallet:
    ...
    async def sign_message(self, message: str, nft: NFTCoinInfo) -> Tuple[G1Element, G2Element]:
        ...
        if uncurried_nft is not None:
            ...
            synthetic_secret_key = calculate_synthetic_secret_key(private, DEFAULT_HIDDEN_PUZZLE_HASH)
            synthetic_pk = synthetic_secret_key.get_g1()
            puzzle: Program = Program.to(("GreenBTC Signed Message", message))
    ...
    async def create_tandem_gbtc_tx(
        self, fee: uint64, announcement_to_assert: Optional[Announcement] = None
    ) -> TransactionRecord:
        greenbtc_coins = await self.standard_wallet.select_coins(fee)
        greenbtc_tx = await self.standard_wallet.generate_signed_transaction(
            uint64(0),
            (await self.standard_wallet.get_new_puzzlehash()),
            fee=fee,
            coins=greenbtc_coins,
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            greenbtc_tx = await self.create_tandem_gbtc_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
    ...
    async def mint_from_did(
        ...
        gbtc_primaries = [
            AmountWithPuzzlehash({"puzzlehash": gbtc_change_ph, "amount": change, "memos": [gbtc_change_ph]})
        ]
        ...
        for gbtc_coin in gbtc_coins:
            ...
            if first:
                ...
                message_list.append(
                    Coin(gbtc_coin.name(), gbtc_primaries[0]["puzzlehash"], gbtc_primaries[0]["amount"]).name()
                )
    ...
    async def mint_from_gbtc(
        ...
        for gbtc_coin in gbtc_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=gbtc_primaries + primaries,
                    fee=fee,
