...
from greenbtc.wallet.util.wallet_sync_utils import fetch_coin_spend
from greenbtc.wallet.util.wallet_types import WalletType
from greenbtc.wallet.wallet import CHIP_0002_SIGN_MESSAGE_PREFIX, Wallet
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSGreenBTCConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSGreenBTCConnection) -> None:
    ...
    async def mint_from_did(
        ...
        if gbtc_coins is None:
            gbtc_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        ...
        gbtc_spend = await self.standard_wallet.sign_transaction(gbtc_spends)
