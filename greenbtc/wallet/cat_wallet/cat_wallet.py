...
from greenbtc.types.coin_spend import compute_additions_with_cost
...
from greenbtc.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, LegacyCATInfo
from greenbtc.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    SpendableCAT,
...
from greenbtc.wallet.cat_wallet.lineage_store import CATLineageStore
from greenbtc.wallet.coin_selection import select_coins
from greenbtc.wallet.conditions import Condition, ConditionValidTimes, UnknownCondition, parse_timelock_info
...
from greenbtc.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            greenbtc_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        greenbtc_tx = dataclasses.replace(greenbtc_tx, spend_bundle=spend_bundle, name=spend_bundle.name())
        await self.standard_wallet.push_transaction(greenbtc_tx)
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSGreenBTCConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_gbtc_tx(
        ...
        if fee > amount_to_claim:
            greenbtc_coins = await self.standard_wallet.select_coins(
                fee,
                tx_config.coin_selection_config,
            )
            origin_id = list(greenbtc_coins)[0].name()
            ...
            origin_id = list(greenbtc_coins)[0].name()
            selected_amount = sum([c.amount for c in greenbtc_coins])
            [greenbtc_tx] = await self.standard_wallet.generate_signed_transaction(
            ...
            message = None
            for spend in greenbtc_tx.spend_bundle.coin_spends:
        ...
        return greenbtc_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some GBTC as part of this
        regular_greenbtc_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_greenbtc_to_claim = payment_amount

        need_greenbtc_transaction = (fee > 0 or regular_greenbtc_to_claim > 0) and (fee - regular_greenbtc_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_greenbtc_transaction:
                    if fee > regular_greenbtc_to_claim:
                        greenbtc_tx, _ = await self.create_tandem_gbtc_tx(
                            fee,
                            uint64(regular_greenbtc_to_claim),
                            tx_config,
                    ...
                    elif regular_greenbtc_to_claim > fee:
                        greenbtc_tx, _ = await self.create_tandem_gbtc_tx(
                            fee,
                            uint64(regular_greenbtc_to_claim),
                            tx_config,
        ...
        greenbtc_spend_bundle = SpendBundle([], G2Element())
        if greenbtc_tx is not None and greenbtc_tx.spend_bundle is not None:
            greenbtc_spend_bundle = greenbtc_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    greenbtc_spend_bundle,
                ]
            ),
            greenbtc_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if greenbtc_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=greenbtc_tx.confirmed_at_height,
                    created_at_time=greenbtc_tx.created_at_time,
                    to_puzzle_hash=greenbtc_tx.to_puzzle_hash,
                    amount=greenbtc_tx.amount,
                    fee_amount=greenbtc_tx.fee_amount,
                    confirmed=greenbtc_tx.confirmed,
                    sent=greenbtc_tx.sent,
                    spend_bundle=None,
                    additions=greenbtc_tx.additions,
                    removals=greenbtc_tx.removals,
                    wallet_id=greenbtc_tx.wallet_id,
                    sent_to=greenbtc_tx.sent_to,
                    trade_id=greenbtc_tx.trade_id,
                    type=greenbtc_tx.type,
                    name=greenbtc_tx.name,
                    memos=[],
                    valid_times=parse_timelock_info(extra_conditions),
                )
            )
