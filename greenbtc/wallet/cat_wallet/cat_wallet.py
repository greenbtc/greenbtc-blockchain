...
from greenbtc.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        greenbtc_tx = dataclasses.replace(greenbtc_tx, spend_bundle=spend_bundle)
    ...
    async def create_tandem_gbtc_tx(
        ...
        if fee > amount_to_claim:
            greenbtc_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            ...
            if first:
                ...
                if need_greenbtc_transaction:
                    if fee > regular_greenbtc_to_claim:
                        ...
                        greenbtc_tx, _ = await self.create_tandem_gbtc_tx(
                            fee, uint64(regular_greenbtc_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
