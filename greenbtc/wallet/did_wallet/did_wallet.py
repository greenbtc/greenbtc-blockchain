...
from greenbtc.util.ints import uint64, uint32, uint8, uint128
...
class DIDWallet:
    ...
    async def coin_added(self, coin: Coin, _: uint32, peer: WSGreenBTCConnection):
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            greenbtc_tx = await self.create_tandem_gbtc_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
