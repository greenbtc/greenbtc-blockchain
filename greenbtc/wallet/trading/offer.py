...
from greenbtc.util.bech32m import bech32_decode, bech32_encode, convertbits
...
from greenbtc.wallet.outer_puzzles import (
    construct_puzzle,
    create_asset_id,
    get_inner_puzzle,
    get_inner_solution,
...
from greenbtc.wallet.util.puzzle_compression import (
...
class Offer:
    ...
    def notarize_payments(
        requested_payments: Dict[Optional[bytes32], List[Payment]],  # `None` means you are requesting GBTC
        coins: List[Coin],
    ...
    def _get_offered_coins(self) -> Dict[Optional[bytes32], List[Coin]]:
        ...
        for parent_spend in self._bundle.coin_spends:
            ...
            if puzzle_driver is not None:
                ...
                # It's much easier if the asset is bare GBTC
                asset_id = None
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "gbtc" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
