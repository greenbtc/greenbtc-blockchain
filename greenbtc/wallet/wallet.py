...
class Wallet:
    ...
    async def create_spend_bundle_relative_greenbtc(self, greenbtc_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if greenbtc_amount < 0:
            utxos = await self.select_coins(uint64(abs(greenbtc_amount)), exclude)
        else:
        ...
        greenbtc_amount = spend_value + greenbtc_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(greenbtc_amount), "memos": []}
                ]
