...
from greenbtc.wallet.puzzles.clawback.metadata import ClawbackMetadata
...
from greenbtc.wallet.puzzles.puzzle_utils import (
...
from greenbtc.wallet.util.wallet_types import WalletIdentifier, WalletType
...
class Wallet:
    ...
    async def create_tandem_gbtc_tx(
        ...
        greenbtc_coins = await self.select_coins(fee, tx_config.coin_selection_config)
        [greenbtc_tx] = await self.generate_signed_transaction(
            uint64(0),
    ...
    # WSGreenBTCConnection is only imported for type checking
