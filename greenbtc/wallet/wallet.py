...
from greenbtc.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
class Wallet:
    ...
    async def create_spend_bundle_relative_greenbtc(self, greenbtc_amount: int, exclude: List[Coin]) -> SpendBundle:
        ...
        if greenbtc_amount < 0:
            utxos = await self.select_coins(abs(greenbtc_amount), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": greenbtc_amount}]
