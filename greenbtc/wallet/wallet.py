...
from greenbtc.full_node.bundle_tools import simple_solution_generator
...
from greenbtc.wallet.secret_key_store import SecretKeyStore
...
# https://github.com/GreenBTC-Network/chips/blob/80e4611fe52b174bf1a0382b9dff73805b18b8c6/CHIPs/chip-0002.md#signmessage
CHIP_0002_SIGN_MESSAGE_PREFIX = "GreenBTC Signed Message"
...
class Wallet:
    ...
    async def create_tandem_gbtc_tx(
        ...
        greenbtc_coins = await self.select_coins(fee)
        if reuse_puzhash is None:
            reuse_puzhash_config = self.wallet_state_manager.config.get("reuse_public_key_for_change", None)
            if reuse_puzhash_config is None:
                reuse_puzhash = False
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSGreenBTCConnection
