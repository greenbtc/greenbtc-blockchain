...
from greenbtc.wallet.puzzle_drivers import Solver
from greenbtc.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import solution_for_conditions
...
from greenbtc.wallet.vc_wallet.cr_cat_drivers import CRCAT, CRCATSpend, ProofsChecker, construct_pending_approval_state
from greenbtc.wallet.vc_wallet.vc_drivers import VerifiedCredential
from greenbtc.wallet.vc_wallet.vc_store import VCProofs, VCRecord, VCStore
...
if TYPE_CHECKING:
    from greenbtc.wallet.wallet_state_manager import WalletStateManager  # pragma: no cover
...
class VCWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSGreenBTCConnection, coin_data: Optional[Streamable]
    ) -> None:
        """
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if fee > 0:
            ...
            greenbtc_tx = await self.wallet_state_manager.main_wallet.create_tandem_gbtc_tx(
        ...
        if greenbtc_tx is not None and greenbtc_tx.spend_bundle is not None:
            spend_bundles.append(greenbtc_tx.spend_bundle)
            tx_list.append(dataclasses.replace(greenbtc_tx, spend_bundle=None))
    ...
    async def revoke_vc(
        ...
        if fee > 0:
            greenbtc_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.create_tandem_gbtc_tx(
                fee, tx_config, vc_announcement
            )
            assert did_tx.spend_bundle is not None
            assert greenbtc_tx.spend_bundle is not None
            new_bundle = SpendBundle.aggregate([greenbtc_tx.spend_bundle, did_tx.spend_bundle])
