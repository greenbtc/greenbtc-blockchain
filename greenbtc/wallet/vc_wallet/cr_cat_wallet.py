...
from greenbtc.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from greenbtc.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from greenbtc.wallet.util.tx_config import TXConfig
...
from greenbtc.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSGreenBTCConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_greenbtc_transaction:
                    if fee > regular_greenbtc_to_claim:
                        ...
                        greenbtc_tx, gbtc_announcement = await self.create_tandem_gbtc_tx(
                            fee,
                            uint64(regular_greenbtc_to_claim),
                            tx_config,
                        )
                        assert gbtc_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={gbtc_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [greenbtc_tx.spend_bundle]
            ...
            [*vc_txs, *([greenbtc_tx] if greenbtc_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        # Select the relevant GBTC coins
        if fee > 0:
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(greenbtc_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if greenbtc_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, greenbtc_tx.spend_bundle])
        else:
            greenbtc_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(greenbtc_tx, spend_bundle=None),) if greenbtc_tx is not None else []),
        ]
