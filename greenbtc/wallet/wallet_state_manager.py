...
from greenbtc.consensus.block_rewards import (
    calculate_base_farmer_reward,
    calculate_pool_reward,
    calculate_stake_farm_reward,
)
from greenbtc.consensus.coinbase import (
    farmer_parent_id,
    pool_parent_id,
...
from greenbtc.data_layer.dl_wallet_store import DataLayerStore
from greenbtc.pools.pool_puzzles import (
...
from greenbtc.types.stake_value import STAKE_FARM_MIN
...
from greenbtc.util.misc import UInt32Range, UInt64Range, VersionedBlob
from greenbtc.util.path import path_from_root
from greenbtc.util.streamable import Streamable
from greenbtc.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from greenbtc.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, CRCATInfo
from greenbtc.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle, match_cat_puzzle
...
from greenbtc.wallet.conditions import Condition, ConditionValidTimes, parse_timelock_info
...
from greenbtc.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
from greenbtc.wallet.derivation_record import DerivationRecord
...
from greenbtc.wallet.did_wallet.did_info import DIDCoinData
from greenbtc.wallet.did_wallet.did_wallet import DIDWallet
from greenbtc.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, match_did_puzzle
from greenbtc.wallet.key_val_store import KeyValStore
from greenbtc.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from greenbtc.wallet.nft_wallet.nft_wallet import NFTWallet
from greenbtc.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from greenbtc.wallet.notification_manager import NotificationManager
from greenbtc.wallet.outer_puzzles import AssetType
from greenbtc.wallet.payment import Payment
from greenbtc.wallet.puzzle_drivers import PuzzleInfo
from greenbtc.wallet.puzzles.clawback.drivers import generate_clawback_spend_bundle, match_clawback_puzzle
from greenbtc.wallet.puzzles.clawback.metadata import ClawbackMetadata, ClawbackVersion
from greenbtc.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_hash_for_synthetic_public_key,
)
from greenbtc.wallet.puzzles.stake.drivers import match_stake_puzzle, generate_stake_spend_bundle
from greenbtc.wallet.puzzles.stake.metadata import StakeMetadata, StakeVersion
from greenbtc.wallet.sign_coin_spends import sign_coin_spends
from greenbtc.wallet.singleton import create_singleton_puzzle, get_inner_puzzle_from_singleton, get_singleton_id_from_puzzle
from greenbtc.wallet.trade_manager import TradeManager
from greenbtc.wallet.trading.trade_status import TradeStatus
from greenbtc.wallet.transaction_record import TransactionRecord, TransactionRecordOld
from greenbtc.wallet.uncurried_puzzle import uncurry_puzzle
from greenbtc.wallet.util.address_type import AddressType
...
from greenbtc.wallet.util.puzzle_decorator import PuzzleDecoratorManager
from greenbtc.wallet.util.query_filter import HashFilter
from greenbtc.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from greenbtc.wallet.util.tx_config import TXConfig, TXConfigLoader
from greenbtc.wallet.util.wallet_sync_utils import (
    PeerRequestException,
    fetch_coin_spend_for_coin_state,
    last_change_height_cs,
)
from greenbtc.wallet.util.wallet_types import CoinType, WalletIdentifier, WalletType
from greenbtc.wallet.vc_wallet.cr_cat_drivers import CRCAT, ProofsChecker, construct_pending_approval_state
from greenbtc.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from greenbtc.wallet.vc_wallet.vc_drivers import VerifiedCredential
from greenbtc.wallet.vc_wallet.vc_store import VCStore
from greenbtc.wallet.vc_wallet.vc_wallet import VCWallet
from greenbtc.wallet.wallet import Wallet
from greenbtc.wallet.wallet_blockchain import WalletBlockchain
from greenbtc.wallet.wallet_coin_record import MetadataTypes, WalletCoinRecord
from greenbtc.wallet.wallet_coin_store import WalletCoinStore
from greenbtc.wallet.wallet_info import WalletInfo
from greenbtc.wallet.wallet_interested_store import WalletInterestedStore
from greenbtc.wallet.wallet_nft_store import WalletNftStore
from greenbtc.wallet.wallet_pool_store import WalletPoolStore
from greenbtc.wallet.wallet_protocol import WalletProtocol
from greenbtc.wallet.wallet_puzzle_store import WalletPuzzleStore
from greenbtc.wallet.wallet_retry_store import WalletRetryStore
from greenbtc.wallet.wallet_transaction_store import WalletTransactionStore
from greenbtc.wallet.wallet_user_store import WalletUserStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSGreenBTCConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            try:
                ...
                # Remove the clawback hint since it is unnecessary for the GBTC coin
        ...
        if fee > 0:
            greenbtc_tx = await self.main_wallet.create_tandem_gbtc_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert greenbtc_tx.spend_bundle is not None
            spend_bundle = SpendBundle.aggregate([spend_bundle, greenbtc_tx.spend_bundle])
        assert derivation_record is not None
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        gbtc_spam_amount = self.config.get("gbtc_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if gbtc_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(gbtc_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < gbtc_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSGreenBTCConnection) -> Optional[bytes32]:
        # Get minter DID
    ...
    async def spend_stake_coins(
        ...
        spend_bundle: SpendBundle = await self.sign_transaction(coin_spends)
        if fee > 0:
            gbtc_tx = await self.main_wallet.create_tandem_gbtc_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert gbtc_tx.spend_bundle is not None
            spend_bundle = SpendBundle.aggregate([spend_bundle, gbtc_tx.spend_bundle])
        assert derivation_record is not None
        tx_record = TransactionRecord(
            confirmed_at_height=uint32(0),
            created_at_time=now,
