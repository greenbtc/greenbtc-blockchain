...
from greenbtc.consensus.blockchain import AddBlockResult
...
from greenbtc.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from greenbtc.protocols.protocol_message_types import ProtocolMessageTypes
from greenbtc.protocols.wallet_protocol import (
    CoinState,
    CoinStateUpdate,
    NewPeakWallet,
...
from greenbtc.rpc.rpc_server import StateChangedProtocol, default_get_connections
from greenbtc.server.node_discovery import WalletPeers
...
from greenbtc.util.config import (
    WALLET_PEERS_PATH_KEY_DEPRECATED,
    lock_and_load_config,
    process_config_start_method,
    save_config,
)
from greenbtc.util.db_wrapper import manage_connection
from greenbtc.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
...
from greenbtc.wallet.puzzles.clawback.metadata import AutoClaimSettings
from greenbtc.wallet.transaction_record import TransactionRecord
from greenbtc.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from greenbtc.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
...
from greenbtc.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    def get_cache_for_peer(self, peer: WSGreenBTCConnection) -> PeerRequestCache:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'greenbtc keys' program.")
            return None
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.greenbtc.top"]),
    ...
    def on_disconnect(self, peer: WSGreenBTCConnection) -> None:
    ...
    async def on_connect(self, peer: WSGreenBTCConnection) -> None:
        if self._wallet_state_manager is None:
            return None
    ...
    async def long_sync(
        ...
        full_node: WSGreenBTCConnection,
        fork_height: int,
        *,
        rollback: bool,
    ) -> None:
    ...
    async def is_peer_synced(self, peer: WSGreenBTCConnection, height: uint32) -> Optional[uint64]:
        # Get last timestamp
    ...
    def is_trusted(self, peer: WSGreenBTCConnection) -> bool:
    ...
    async def state_update_received(self, request: CoinStateUpdate, peer: WSGreenBTCConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSGreenBTCConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSGreenBTCConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSGreenBTCConnection]:
        """
        ...
        synced_and_trusted: List[WSGreenBTCConnection] = []
        synced: List[WSGreenBTCConnection] = []
        trusted: List[WSGreenBTCConnection] = []
        neither: List[WSGreenBTCConnection] = []
        all_nodes: List[WSGreenBTCConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSGreenBTCConnection) -> None:
        if self._wallet_state_manager is None:
        ...
        difficulty_coefficient: Decimal = Decimal(response.difficulty_coefficient)
        ...
        if self.is_trusted(peer):
            ...
            if not await self.new_peak_from_untrusted(new_peak_hb, difficulty_coefficient, peer):
                return
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSGreenBTCConnection
    ) -> None:
    ...
    async def new_peak_from_untrusted(
        self, new_peak_hb: HeaderBlock, difficulty_coefficient: Decimal, peer: WSGreenBTCConnection
    ) -> bool:
        far_behind: bool = (
        ...
        if new_peak_hb.height < self.constants.WEIGHT_PROOF_RECENT_BLOCKS:
            ...
            return await self.sync_from_untrusted_close_to_peak(new_peak_hb, difficulty_coefficient, peer)
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSGreenBTCConnection) -> None:
    ...
    async def sync_from_untrusted_close_to_peak(
        ...
        async with self.wallet_state_manager.lock:
            ...
            if peak_hb is None or new_peak_hb.weight > peak_hb.weight:
                backtrack_fork_height: int = await self.wallet_short_sync_backtrack(
                    new_peak_hb, difficulty_coefficient, peer
                )
            else:
    ...
    async def wallet_short_sync_backtrack(
            self, header_block: HeaderBlock, difficulty_coefficient: Decimal, peer: WSGreenBTCConnection
    ) -> int:
        peak: Optional[HeaderBlock] = await self.wallet_state_manager.blockchain.get_peak_block()

        top = header_block
        blocks = [(top, difficulty_coefficient)]
        ...
        while not self.wallet_state_manager.blockchain.contains_block(top.prev_header_hash) and top.height > 0:
            ...
            blocks.append((prev_head, Decimal(response_prev.difficulty_coefficient)))
            top = prev_head
        ...
        for block, difficulty_coefficient in blocks:
            # Set blockchain to the latest peak
            res, err = await self.wallet_state_manager.blockchain.add_block(block, difficulty_coefficient)
    ...
    async def fetch_and_update_weight_proof(self, peer: WSGreenBTCConnection, peak: HeaderBlock) -> int:
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSGreenBTCConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSGreenBTCConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSGreenBTCConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
