...
from greenbtc.util.ints import uint16, uint32, uint64, uint128, uint8
...
class WalletNode:
    ...
    async def get_timestamp_for_height_from_peer(self, height: uint32, peer: WSGreenBTCConnection) -> Optional[uint64]:
        """
    ...
    async def new_peak_from_untrusted(self, new_peak_hb: HeaderBlock, peer: WSGreenBTCConnection) -> bool:
        far_behind: bool = (
    ...
    async def sync_from_untrusted_close_to_peak(self, new_peak_hb: HeaderBlock, peer: WSGreenBTCConnection) -> bool:
        async with self.wallet_state_manager.lock:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSGreenBTCConnection) -> int:
