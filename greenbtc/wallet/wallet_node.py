...
from greenbtc.consensus.blockchain import ReceiveBlockResult
from greenbtc.consensus.constants import ConsensusConstants
from greenbtc.daemon.keychain_proxy import (
    KeychainProxy,
    connect_to_keychain_and_validate,
    wrap_local_keychain,
)
from greenbtc.protocols import wallet_protocol, farmer_protocol
...
from greenbtc.util.chunks import chunks
from greenbtc.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from greenbtc.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
...
from greenbtc.wallet.wallet_weight_proof_handler import get_wp_fork_point, WalletWeightProofHandler
...
class WalletNode:
    ...
    def set_server(self, server: GreenBTCServer):
        self._server = server
        self.initialize_wallet_peers()
    ...
    def on_disconnect(self, peer: WSGreenBTCConnection):
    ...
    async def on_connect(self, peer: WSGreenBTCConnection):
        if self._wallet_state_manager is None:
            return None
    ...
    async def is_peer_synced(
        self, peer: WSGreenBTCConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSGreenBTCConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSGreenBTCConnection]:
        """
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSGreenBTCConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSGreenBTCConnection):
        if self._wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(
        self, header_block: HeaderBlock, peer: WSGreenBTCConnection
    ) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSGreenBTCConnection, peak: HeaderBlock
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSGreenBTCConnection) -> CoinSpend:
