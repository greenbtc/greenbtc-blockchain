...
from greenbtc.util.ints import uint8, uint32, uint64, uint16
...
from greenbtc.wallet.nft_wallet.nft_info import NFTInfo, NFTCoinInfo
...
class WalletRpcApi:
    ...
    async def check_offer_validity(self, request) -> EndpointResult:
        offer_hex: str = request["offer"]
        offer = Offer.from_bech32(offer_hex)
        peer: Optional[WSGreenBTCConnection] = self.service.get_full_node_peer()
        if peer is None:
