...
from greenbtc.rpc.util import wrap_http_handler
from greenbtc.server.outbound_message import NodeType
from greenbtc.server.server import GreenBTCServer, ssl_context_for_client, ssl_context_for_server
...
from greenbtc.util.config import str2bool
from greenbtc.util.ints import uint16
from greenbtc.util.json_util import dict_to_json_str
from greenbtc.util.network import WebServer, resolve
from greenbtc.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> GreenBTCServer:
        ...
        # a property so as to be read only which allows GreenBTCServer to satisfy
        # Optional[GreenBTCServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        greenbtc.rpc.rpc_server.default_get_connections()
        """
        ...
...
def default_get_connections(server: GreenBTCServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
