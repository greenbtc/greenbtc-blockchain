...
# Gets the version from the result of "greenbtc version"
...
def get_greenbtc_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["greenbtc", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_greenbtc_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/greenbtc-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/greenbtc-blockchain-gui/packages/gui/package.json")
